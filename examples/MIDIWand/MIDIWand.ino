...
AudioConnection          patchCord5(fade1, 0, usb1, 0);
AudioConnection          patchCord6(fade1, 0, usb1, 1);
// GUItool: end automatically generated code

int batCounter = 0;

int midiNote = 0, lastMidiNote=0;
...
float logZ(float x, float z)  { return log(x) / log(z); }
...
void loop() {
  ...
  if( now - lastNoteUpdate > 10 ){
    ...
    if (!ignore) {
      ...
      midiNote = 12.0*(log(freq/440.0)/log(2))+69;
    ...
    case 2:
      if ( val && !playing  ) {
        ...
        if( !playLock ){
          playPressed = false;
          playing = false;
          usbMIDI.sendNoteOff(lastMidiNote, 0, 1);
          fade1.fadeOut(100);
        }
      }
      break;
...
//////////////////////////////////////
    case 10:
      if( val && !s11pressed ){
        ...
        memset(arbitraryWaveform, 0, 256*sizeof(uint16_t));
